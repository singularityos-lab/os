...
# source) with Buildroot's host go, whose module cache already holds the single dependency,
# so the offline cross-build finds it. Runs before the Buildroot go build.
...
# atomd + updated real binaries -> /usr/libexec/*.real; the rootfs overlay installs
# portable-mode guards at their public paths. fw-verify -> /usr/bin
...
define ATOMLOOPS_INSTALL_TARGET_CMDS
	$(INSTALL) -D -m 0755 $(@D)/bin/atomd     $(TARGET_DIR)/usr/libexec/atomd.real
	$(INSTALL) -D -m 0755 $(@D)/bin/updated   $(TARGET_DIR)/usr/libexec/updated.real
